...
//  SeSAC2Week6
//
//  Created by Brother Model on 2022/08/11.
//
...
class MapViewController: UIViewController {
    ...
    /*
     - 지도와 위치 권한은 상관없음
     - 만약에 지도에 현재 위치 등을 표현하고 싶다면 위치 권한을 등록해주어야 함
     - 중심, 범위 지정
     - 핀(어노테이션)
     */
    
    /*
     권한 : 반영이 조금씩 느릴 수 있음. 지웠다가 실행한다고 하더라도... 한번 허용으로 테스트
     설정 : 앱이 바로 안뜨는 경우도 있음
     */
...
extension MapViewController {
    ...
    func checkUserCurrentLocationAuthorization(_ authorizationStatus: CLAuthorizationStatus) {
        ...
        case .authorizedWhenInUse:
            print("WHEN in USE")
            //사용자가 위치를 허용해준 상태라면, startUpdatingLocation을 통해 didUpadateLocation 메서드가 실행
            locationManager.startUpdatingLocation()
...
extension MapViewController: CLLocationManagerDelegate {
    ...
    //Location5. 사용자의위치를 성공적으로 가지고 온 경우
...
//        setRegionAndAnnotation(center: locations)
        
        //위치 업데이트 멈춰!
        locationManager.stopUpdatingLocation()
    }
    
    //Location6. 사용자의 위치를 성공적으로 가지고 오지 못한 경우
    ...
    //iOS 14 이상: 사용자의 권한 상태가 변경이 될 때, 위치 관리자 생성할 때도 호출 됨
